...
function render() {
  ...
  const tbody=el('videos');tbody.replaceChildren();for(const r of rows){const tr=node('tr');const td=node('td');const a=node('a','video-link',r.now?.title||r.v.portfolioLabel);a.href=r.v.url;a.target='_blank';a.rel='noopener';td.append(node('span',`platform ${r.v.platform}`,r.v.platform==='youtube'?'YouTube':r.v.platform==='tiktok'?'TikTok':'Instagram'),a,node('span','sub',r.now?`Collected ${dateTime(r.now.collectedAt)}${r.now.viewsObservedAt && r.now.viewsObservedAt !== r.now.collectedAt ? ` · Plays last verified ${dateTime(r.now.viewsObservedAt)}` : ""}`:'Metrics unavailable for this date'));tr.append(td);for(const key of ['views','likes','comments']){const cell=node('td');cell.append(node('strong',r.now?.[key]==null?'missing':'',number(r.now?.[key])));const d=metricDelta(r.now,r.prior,key);cell.append(node('span',`sub ${d==null?'':d<0?'down':'up'}`,d==null?'—':`${d>=0?'+':''}${number(d)}`));tr.append(cell);}tbody.append(tr);}if(!rows.length)tbody.append(node('tr','',''));
